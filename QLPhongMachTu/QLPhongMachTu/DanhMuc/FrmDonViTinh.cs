...
namespace QLPhongMachTu.DanhMuc
{
    ...
    {
        private DonViTinhBUS bus = new DonViTinhBUS();
        private DonViTinhDTO itemIndex;
        ...
        {
            itemIndex = new DonViTinhDTO(_id, txtTen.Text.Trim());
        }
        ...
        private bool ThieuDuLieu(bool isInsert)
        {
            if (txtTen.Text.Trim() == "")
            {
                MessageBox.Show("Vui lòng nhập tên đơn vị tính!", "Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                txtTen.Focus();
                return true;
            }
